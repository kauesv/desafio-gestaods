...
def validate_cpf(value):
    ...
    if len(cpf) != 11:
        raise ValidationError({"message": 'CPF deve ter 11 dígitos.'})

    # Verifica números repetidos
    if cpf == cpf[::-1]:
        raise ValidationError({"message": 'CPF inválido: Todos os dígitos são iguais.'})
    ...
    if cpf[9] != digito1 or cpf[10] != digito2:
        raise ValidationError({"message": 'CPF inválido: Dígitos verificadores incorretos.'})